...
| `STUMBLESTACK_EMBED_PROVIDER` | Opt-in semantic search. `none` (default) → lexical. `local-hash` → deterministic test embedding. A real provider + a matching `embeddings.json` enables cosine ranking. | `local-hash` |
